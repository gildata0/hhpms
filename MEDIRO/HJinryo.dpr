...
uses
  ...
  Prt_wonwe in 'wonwe\Prt_wonwe.pas' {Prt_Wonwe_f},
...
begin
     ...
     Mutex := CreateMutex(nil, True, 'HHPMSDOCDORSuccess');
  ...
  with TLogin_f.Create(Application) do
          try
              ...
              begin
               ...
               Application.CreateForm(TJinryo_f, Jinryo_f);

               lblStatus.Caption := '  ' +
                    'HHPMS[진료실]를 시작 하는 중입니다. 잠시 기다리세요.';
               lblStatus.Caption :=
                    '  아이디와 패스워드를 입력하세요.';
              end
              else
     ...
     begin
         Application.MessageBox('HPMS[진료실]가 이미 실행중입니다', '확인', 0);

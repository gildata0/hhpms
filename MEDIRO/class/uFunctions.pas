...
function JuminToGender3(TheInput: string): string; //'O'추가
...
function JuminToBirth3(TheInput: string): string; //년월일 표시
...
procedure KillProcess(strExeName: string); //대소문자 구별한다....
...
//이미지를 폴더나 바탕화면으로 드래그하여 저장하기위해서  GetFileListDataObject 을 사용한다.
...
function FindString(Target: string): string;   //문자만 추출, 수가 가산 코드 만들때.수가 대분류코드 가져오기

procedure NoDup(AListBox: TListBox); //ListBox중복제거
...
var
     ...
     vViewAll : boolean; //메인에서 불러오는 varViewAll
     vFindName : string; //예약 검색을 위한 메인에서 검색한 이름
...
begin
     ...
     Result := iJuminYear + '년 ' + copy(theInput, 3, 2) + '월 ' + copy(theInput,
          5, 2) +'일';
end;
...
begin
       case ECode of
              1: result := '1. 호출요청성공(전송성공)';
              11: result := '11. 전화번호 이상';
              21: result := '21. Connect 실패';
              22: result := '22. 데이터 Send 실패';
              23: result := '23. Ack Receive 실패';
              40: result := '40. SMS 서버 이상';
              41: result := '41. 미등록 사용자';
              67: result := '67. 잔액부족';
              68: result := '68. 고객사 코드 이상';
              73: result := '73. 미등록 회원 또는 기본료미납 ';
              77: result := '77. 호출할 메시지의 내용이 없음';
              78: result := '78. 국번 or 전화번호 뒤 4 자리 이상';
              79: result := '79. 호출요청성공';
              80: result := '80. 이통사번호 이상';
              82: result := '82. ‘700’, ’800’ 금지업체';
              84: result := '84. 예약일자 이상';
              85: result := '85. 호출 URL이 없음';
              88: result := '88. 데이터 포맷 오류 및 기타오류';
              99: result := '99. 데이터 포맷 오류 및 기타오류';
       end;
...
begin
  ...
  try
    ...
    on E: Exception do
    begin
      ExceptLogging('업데이트 서버 접속 실패.' + #10#13 + E.Message);
      Showmessage('업데이트 서버 접속 실패. 인터넷 연결 상태를 확인하세요.' + #10#13 + E.Message);
      mStream.Free;
      Result := false;
      // exit;
    end;
      ...
      begin
          ...
          begin
            if lowercase( extractFileExt(varName)) ='.bmp' then
             ...
             begin
                  showmessage('지원하지 않는 형식입니다.'+ #10#13+
                       '윈도우즈 그림판을 이용해서 복사할수 있습니다.');
                  exit;     
             end;
          end;
          // 2. 비트맵 -> JPEG
...
var
     ...
     rt: DWORD;
begin
     StrPCopy(lpszLocalName, varDrive); // 로컬 드라이브명
...
begin
  ...
  try
    if profile.FirewallEnabled then begin
      ...
      try
        app.ProcessImageFileName := exePathName;//실행파일_패스포함;
        app.Name := appName;
...
begin
     ...
     StrPCopy(HeadMark.Mark, ENCRYPT_MARK); // 암호화된 파일을 표시
...
begin
  ...
  begin
    ...
    if aGrid.Cells[aCol, i] = '' then // 상병코드
    begin
      iRow := i;
      Break;
    end;
